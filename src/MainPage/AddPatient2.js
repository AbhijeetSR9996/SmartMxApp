...
const AddPatient2 = ({navigation, route}) => {
  ...
  const [discharged, setDischarged] = useState('Dischatged');
  ...
  const handlePostData = async () => {
    try {
      ...
      navigation.navigate('UnitA', {
        res: userData,
        grpId: route.params.grpId, // Pass the grpId from the previous screen
        grpName: route.params.grpName, // Pass the grpName from the previous screen
      });
    } catch (error) {
      console.error(error);
    }
