...
const UnitWorkSpace = ({navigation, route}) => {
  ...
  const [groupadmin, setGroupAdmin] = useState(userData.id);
  ...
  useEffect(() => {
    fetchGroupData();
  }, [isFocused]); // Add `isFocused` as a dependency to re-fetch data when the component is focused


  useEffect(() => {
    // fetchApiFullDetails(); // Fetch data from the first API
    fetchGroupData(); // Fetch data from the second API
  }, [isFocused]);
  
  // const fetchApiFullDetails = async () => {
  //   try {
  //     const response = await axios.get('https://smart.techpanda.art/groupdata/61/');
  ...
  const addGroup = async () => {
    try {
      ...
      const response = await axios.post(
        ...
        {
          group: group,
          groupadmin: groupadmin, // Set the groupadmin as groupadmin
        },
      );
      ...
      setGroup('');
      setShowModal(false);
      Alert.alert('Success', 'The New group has been added successfully.');
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const fetchGroupData = async () => {
    try {
      ...
      const groupData = response.data.filter(item => {
        return (
          item.groupadmin === userData.id ||
          item.assigndoctor.includes(userData.id)
        );
      });
