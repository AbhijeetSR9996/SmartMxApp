...
const UnitA = ({navigation, route}) => {
  const userData = route.params.res; //Login User Data
  ...
  useFocusEffect(
    React.useCallback(() => {
      // Execute this callback when the screen comes into focus
      setRefresh(true); // Trigger the refresh by updating the "refresh" state
    }, []),
  );
  ...
  const PatientCards = ({item, index}) => {
    if (
      ...
      return null; // Skip rendering if the name does not match the filter
    }
    ...
    const [apiFullDetails, setApiFullDetails] = useState(false);

    useEffect(() => {
      fetchApiFullDetails(); // Fetch data from the first API
    }, [userData.id, grpId , refresh]);

    const fetchApiFullDetails = async () => {
      try {
        const response = await axios.get(
          `https://smart.techpanda.art/groupdata/${grpId}/`,
        );
        setApiFullDetails(response.data.results);
    ...
    console.log('apiFullDetails-------------------------------------------------------', apiFullDetails);

    return (
      <View
        style={{
          // backgroundColor: item.status ? '#06C270' : '#FFA500',
          backgroundColor: item.status ? '#D5FFEF' : '#FAD6CB',
          flexDirection: 'row',
          alignItems: 'center',
          padding: 16,
          borderRadius: 20,
  ...
  const executeDelete = grpId => {
    axios
      ...
      .then(response => {
        ...
        navigation.navigate('UWorkSpace', {res: userData, refresh: true});
      })
      .catch(error => {
        console.error(error);
      });
    ...
    <SafeAreaView style={styles.container}>
      <ImageBackground
        ...
        <View style={styles.userInfo}>
          ...
          <View style={[styles.filterContainer2, {marginTop: -5}]}>
            <TextInput
              value={filterValue}
        ...
        {loading ? (
          ...
          <View style={styles.containerLoadingCircle}>
          ...
          // Render FlatList when data is fetched
          <>
            {getPatient.length > 0 ? (
              <FlatList
                data={getPatient}
              ...
              <View style={styles.noDataFoundTextContainer}>
                <Text style={styles.noDataFoundText}>No Details Found</Text>
              </View>
            )}
          </>
        )}
        ...
        {loading ? (
          <View style={styles.buttonContainer}>
            <TextButton
              ...
              icon={icons.plus}
              onPress={() =>
                navigation.navigate('AddPatient2', {
                  res: userData,
                  grpId: grpId,
                  grpName: grpName,
                })
          ...
          <View style={{marginHorizontal: 20, marginBottom: 20, marginTop: 16}}>
            <TextButton
              title="Add New Patient details"
              bgColor="#6D21A9"
              color="#ffffff"
